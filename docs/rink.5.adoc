...
*show_interpretation* = <__bool__>::
	Shows the input query reformatted based on how Rink interpreted it.
...
*keyword* = <__color__>::
	Special words in Rink's query language, like `of`, `digits`, etc.
	Default: `"yellow"`

*timezone* = <__color__>::
	Timezones, like `[US/Pacific]`.
	Default: `"cyan"`
